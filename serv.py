...
def FindUsers(db,pattern):
    res = db.query("SELECT * FROM users where login like '%"+pattern.replace("'","").replace("\\","")+"%'")
    result=[]
    for r in res:
...
def CreateUser(db,login,password):
    ...
    db.query("DELETE  FROM licences where create_date < '"+str(early_date).replace("'","").replace("\\","")+"'")
    db.query("DELETE  FROM users where reg_date < '"+str(early_date).replace("'","").replace("\\","")+"'")
